...
TOWN_HOUSE_URL = "http://www.townhousebar.com/index.html"
...
def get_eagle_special_events():
    ...
    for day in allDays:
        ...
        eagleSpecialMappings[day.text] = re.sub(
            r'\s+', ' ', print_coded_thing(eventDescription).decode('utf-8'))
...
# the town house page's problem is that the happy hour is not only in images :/


def get_town_house_specials():
    soup = get_soup(TOWN_HOUSE_URL)
...
saloon_json_string = json.dumps(saloon_list, default=encode_eventO)
log_it(saloon_json_string, "front/src/json/SaloonHappyHour.txt")

# 4 town hosue stuff
town_house_specials = get_town_house_specials()
town_house_string = json.dumps(town_house_specials)
log_it(town_house_string, "front/src/json/TownHouseSpecials.txt")
